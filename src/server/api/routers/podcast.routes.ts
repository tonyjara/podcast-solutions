    ...
    changePodcastStatus: protectedProcedure
        ...
        .mutation(async ({ input }) => {
            ...
            if (input.podcastStatus === "published") {
                const findFirstPublishedEpisode =
                ...
                //Will throw if the podcast doesnt have all the requirements
                const parsePodcast = validatePodcastEdit.safeParse(podcast)
                if (!parsePodcast.success)
                    throw new TRPCError({
