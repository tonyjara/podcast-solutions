...
export default function Pricing({ prices, products }: PricingPageProps) {
    ...
    return (
        <Container id="pricing" py={12} maxW={"5xl"}>
            <VStack spacing={2} textAlign={{ base: "start", md: "center" }}>
                <Heading maxW="800px" as="h1" fontSize="4xl">
                    Choose the plan that better fits your needs{" "}
                </Heading>
                <Text maxW="800px" fontSize="lg">
                    Cancel any time, no questions asked. All values are
                    cumulative, if you don&apos;t use them they remain in your
                    account for as long as your subscription is active, in case
                    of suspending your subscription your values will be held as
                    is for 3 months.
                </Text>
            </VStack>
            <Flex
                    ...
                    .map((product, i) => {
                        ...
                        return (
                            <PricingCard
                                ...
                                key={product.id}
                                payAsYouGo={
                                    payAsYouGo ? payAsYouGo.split(",") : []
                                }
                                handleCheckout={() => {
                                    if (!product.default_price || !product.id)
                                        return
